	...
	std::vector<GameObject>& gameObjects)
{
	...
	for (auto &it:bp.getComponentList())
	{
		if (it == "Transform")
		{
		...
		else if (it == "Animation Graphics") {
			shared_ptr<AnimationGraphicsComponent> agp =
				make_shared<AnimationGraphicsComponent>();

			gameObject.addComponent(agp);
			agp->initializeGraphics(
				bp.getBitmapName(),
				Vector2f(bp.getWidth(),
					bp.getHeight()));
