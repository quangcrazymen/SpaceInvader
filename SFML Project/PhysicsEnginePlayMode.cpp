	...
	const vector<int>& explosionPositions)
{
	...
	for (auto &invaderIt:objects)
	{
			...
			&& invaderIt.getTag() == "invader")
		{
			...
			auto explosionIt = objects.begin();
			// HARDCODE THE NUMBER OF EXPLOSION FIRST
			//if (m_CurrentExplosionIndex > 3) m_CurrentExplosionIndex = 0;
			advance(explosionIt, explosionPositions[m_CurrentExplosionIndex]);
			// Item iterator
			//auto itemIt = objects.begin();
			//advance(itemIt,)
			//adva
...

			{
					...
					->m_BelongsToPlayer)
				{
					...
					//explosionIt->setActive();
					explosionIt->getTransformComponent()->getLocation() 
						= invaderIt.getTransformComponent()->getLocation();
			...
			if (currentCollider.intersects(playerCollider))
			{
				if (it3.getTag() == "bullet")
				{
					...
					it3.getTransformComponent()->
						getLocation() = offScreen;
					// send some special graphics here
				}
...
void PhysicsEnginePlayMode::detectPlayerAndItemCollisions(
	vector<GameObject>& objects,
...
{
	detectInvaderCollisions(objects, bulletPositions,explosionPositions);
	detectPlayerCollisionsAndInvaderDirection(
		objects, bulletPositions);
	//detectPlayerAndItemCollisions(objects, itemPositions);
